...
import type { SchemeActionKind } from './useSchemeHistory';
import {
  buildParentMap,
  collectDescendants,
  findForestNode,
...
export function useRegionMutations(deps: {
  ...
  /** Block edit when viewing past history; truncate future branch after confirm. */
  ensureCanMutate: () => Promise<boolean>;
  /** Record one history entry after a successful mutation. */
  recordHistoryAction: (scheme: Scheme, meta: SchemeActionMeta) => void;
  /** Skip per-field history entries during region panel batch save. */
  isHistoryBatch: () => boolean;
}) {
  const { t } = useI18n();
  const {
    scheme,
  ...
  const maybeRecord = useCallback((next: Scheme, meta: SchemeActionMeta) => {
    if (!isHistoryBatch()) recordHistoryAction(next, meta);
  }, [isHistoryBatch, recordHistoryAction]);
  ...
  const handleAddManual = useCallback(async (data: {
    ...
    try {
      await runBusy(t('status.building'), async () => {
        ...
        maybeRecord(result.scheme, {
          kind: 'addRegion',
          labelKey: 'history.addRegion',
          labelParams: { id: data.id },
        });
        closeAddDialog();
  ...
  const handleUpdateFlags = useCallback(async (
    ...
    let nextScheme: Scheme | null = null;
    setScheme((prev) => {
      if (!prev) return prev;
      nextScheme = {
        ...prev,
    ...
    if (nextScheme) {
      maybeRecord(nextScheme, {
        kind: 'updateFlags',
        labelKey: 'history.updateFlags',
        labelParams: { id: regionId },
      });
    }
    setStatus(t('status.flagsUpdated', { id: regionId }));
  }, [t, setScheme, setStatus, ensureCanMutate, maybeRecord]);
  ...
  const handleUpdateParent = useCallback(async (
    ...
    maybeRecord(result.scheme, {
      kind: 'updateParent',
      labelKey: 'history.updateParent',
      labelParams: { id: regionId },
    });
    setStatus(t('status.parentUpdated', { id: regionId }));
  }, [t, applyScheme, collapseThreshold, setStatus, ensureCanMutate, maybeRecord]);
  ...
  const handleUpdateGeometry = useCallback(async (
    ...
    maybeRecord(result.scheme, {
      kind: 'updateGeometry',
      labelKey: 'history.updateGeometry',
      labelParams: { id: regionId },
    });
  ...
  const handleRename = useCallback(async (regionId: string, newId: string) => {
    ...
    maybeRecord(result.scheme, {
      kind: 'renameRegion',
      labelKey: 'history.renameRegion',
      labelParams: { oldId: regionId, id: normalized },
    });
    setDetailsNav((prev) => {
  ...
  }, [t, applyScheme, collapseThreshold, setDetailsNav, setSelectedId, setCollapseTarget, setHiddenNodes, setStatus, ensureCanMutate, maybeRecord]);
  ...
  const handleUpdatePriority = useCallback(async (regionId: string, priority: number) => {
    ...
    if (nextScheme) {
      maybeRecord(nextScheme, {
        kind: 'updatePriority',
        labelKey: 'history.updatePriority',
        labelParams: { id: regionId },
      });
    }
  ...
  const handleUpdateMembers = useCallback(async (
    ...
    if (nextScheme) {
      maybeRecord(nextScheme, {
        kind: 'updateMembers',
        labelKey: 'history.updateMembers',
        labelParams: { id: regionId },
      });
    }
  ...
  const handleBulkFlags = useCallback(async (payload: {
    ...
    if (!await ensureCanMutate()) return { count: 0 };
    ...
    if (nextScheme) {
      const labelKey = payload.action === 'delete'
        ? 'history.bulkFlagsDelete'
        : 'history.bulkFlagsUpdate';
      maybeRecord(nextScheme, {
        kind: 'bulkFlags',
        labelKey,
        labelParams: { flag: payload.flag, count: result.count },
      });
    }
  ...
  const handleClearAllFlags = useCallback(async () => {
    ...
    if (nextScheme) {
      maybeRecord(nextScheme, {
        kind: 'clearAllFlags',
        labelKey: 'history.clearAllFlags',
        labelParams: { count: result.count },
      });
    }
    if (highlightFlag) {
    ...
    return { count: result.count };
  }, [t, highlightFlag, applyHighlightFlag, setScheme, setStatus, ensureCanMutate, maybeRecord]);
